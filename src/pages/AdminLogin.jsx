...
const AdminLogin = () => {
  ...
  // Prevents UI flash while navigation is in progress
  const [redirecting, setRedirecting] = useState(false);

  // ── Guard: already-logged-in admin lands on /admin-login ──────────────
  // Only fires after AuthContext has fully resolved (loading = false).
  // Prevents premature redirects while role is still being fetched.
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      if (!auth) throw new Error('Firebase Auth is not initialised. Check your .env file.');
      ...
      // Step 3 — Fetch role from Firestore: users/{uid}
      console.log('[Login] Fetching Firestore document: users/' + uid);
      const snap = await getDoc(doc(db, 'users', uid));
      const role = snap.exists() ? (snap.data().role ?? null) : null;
      console.log('[Login] Firestore doc exists:', snap.exists(), '| role:', role);

      // Step 4 — Route based on role
      if (role === 'admin') {
        console.log('[Login] ✅ Role is admin → navigating to /dashboard');
        setRedirecting(true);           // block UI flash before navigate fires
        navigate('/dashboard', { replace: true });
      } else {
        ...
        setError(
          role
            ...
            : 'Access denied. No role found in Firestore for this user.\n' +
            'Ensure users/{uid} exists with field: role = "admin".'
        );
      }
    } catch (err) {
  ...
  // Block rendering while AuthContext resolves or navigation is in flight
